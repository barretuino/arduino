 ...
 * Classe que modela conceitualmente a Medição de Energia
...
import br.puc.ibarreto.gui.frmGraficoReal;
...
public class Medicao implements SerialPortEventListener {
	...
	public Medicao(String portaCOM, int timeOut, int dataRate) throws SQLException{
		...
		TIME_OUT = timeOut;
		DATA_RATE = dataRate;
		grafico = new frmGraficoReal("Medida de Corrente");		
		grafico.pack();
	...
	**/
				...
				for(int i=0; i<available; i++){
					if(chunk[i]=='$')
						...
						if(chunk[i]=='#'){
							...
							float corrente = Float.parseFloat(texto);

							//Escrevendo as variáveis monitoradas
							leitura.setCorrente(corrente);

							//Realizando Persistencia no Banco de Dados	
							dao.adiciona(leitura);
							System.out.println("Gravação realizada na MIB: " + leitura.getCorrente());
							grafico.setCorrente(corrente);							
						}else
	...
	/*public static void main(String[] args) throws Exception {
		Medicao main = new Medicao("COM3", 2000, 9600);
		main.initialize();
