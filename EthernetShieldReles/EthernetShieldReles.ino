...
int led4 = 0; //armazena o estado do led 0 - low e 1 - high
...
#define pin_rly4 5 //Representa a porta digital 5

byte mac[] = { 0x90, 0xA2, 0xDA, 0x00, 0x9B, 0x36 }; //Mac Adress - Enderço Físico
byte ip[] = { 10, 11, 6, 199 }; //IP da Shield na Rede
byte gateway[] = { 10, 11, 0, 1 }; //IP do Roteador que prôve o acesso
  ...
  if (client)
  {
    ...
    while (client.connected())
    {
      if (client.available())
      {
        ...
        if(str.endsWith("/4on")) led4 =1;
        else if(str.endsWith("/4off")) led4 =0;
